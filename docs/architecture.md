...
**EAGLE-I Outage Data:**
CSV files from the US DOE EAGLE-I system are uploaded to Google Cloud Storage, then loaded into BigQuery via `bq load`. The raw data has 15-minute granularity at the county level with fields: FIPS code, county name, state, customers without power, total customers, and timestamp.
